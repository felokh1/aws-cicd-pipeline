# Resources: https://registry.terraform.io/providers/hashicorp/aws%20%20/latest/docs/resources/codepipeline
...
# In addition to being able to run code pipeline. Our pipeline needs the ability to initiate the codestar connections to github, needs the ability to write artifacts to s3
# write to cloudwatch, write logs, need to trigger code build
# Therefore, you needs to setup permissions to do this by setting up a policy and attaching to the codepipline role above
...
# attaching the Code Build policy to the Code build role
